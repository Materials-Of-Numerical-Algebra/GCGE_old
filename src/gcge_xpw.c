 ...
 *  GCGE_OrthonormalizationInSubspace(coef, ldc, p_start, &p_end, NULL, -1, orth_para)
...
void GCGE_ComputeP(GCGE_DOUBLE *subspace_evec, void **V, GCGE_OPS *ops, GCGE_PARA *para, GCGE_WORKSPACE *workspace)
{
    ...
    {
        GCGE_SCBOrthonormalizationInSubspace(coef + orth_start*ldc + orth_start, 
                ldc, orth_length, p_start, &p_end, NULL, -1, 
                para->orth_para, workspace, ops);
    }
    else if(strcmp(para->p_orth_type, "bgs") == 0)
    {
        GCGE_BOrthonormalizationInSubspace(coef + orth_start*ldc + orth_start, 
                ldc, orth_length, p_start, &p_end, NULL, -1, 
                para->orth_para, workspace->subspace_dtmp, ops);
    }
    else if(strcmp(para->p_orth_type, "multi") == 0)
    {
        GCGE_StableMultiOrthonormalizationInSubspace(coef+orth_start*ldc + orth_start, 
                ldc, orth_length, p_start, &p_end, NULL, -1, 
                para->orth_para, workspace, ops);
    }
    else
