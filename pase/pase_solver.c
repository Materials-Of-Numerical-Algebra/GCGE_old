        ...
        PASE_INT block_size, PASE_PARAMETER param, GCGE_OPS *gcge_ops)
{
  ...
  PASE_Mg_set_up(solver, A, B, evec, param);
 ...
 */
PASE_MG_SOLVER
PASE_Mg_solver_create(void *A, void *B, PASE_PARAMETER param)
{
 ...
 */
PASE_INT
PASE_Mg_set_up(PASE_MG_SOLVER solver, void *A, void *B, void **x, PASE_PARAMETER param)
{
  clock_t start, end;
  start = clock();
  PASE_INT i = 0;
  ...
  solver->gcge_ops->MultiVecCreateByMat(&(solver->u), block_size, A, solver->gcge_ops);
  /* TODO 给u_tmp1也分配了nev大小的向量组空间 */
  solver->gcge_ops->MultiVecCreateByMat(&(solver->u_tmp), block_size, A, solver->gcge_ops);
  ...
  if(NULL != solver) {
    ...
    if(NULL != solver->aux_u) {
      for(i = 0; i < solver->nleve; ++i) {
        if(NULL != solver->aux_u[i]){
            solver->pase_ops->MultiVecDestroy(&(solver->aux_u[i]), solver->block_size, solver->pase_ops);
        }
      }
    ...
    if(NULL != solver->u) {
      if(PASE_YES == solver->is_u_owner) {
          solver->gcge_ops->MultiVecDestroy(&(solver->u), solver->block_size, solver->gcge_ops);
      }
...
PASE_Mg_error_estimate(PASE_MG_SOLVER solver)
{
  ...
  void            *A0	         = solver->multigrid->A[0];
  void            *B0	         = solver->multigrid->B[0];
  ...
  for(i = nconv; i < block_size; ++i) {
    ...
    gcge_ops->VecInnerProd(ui, Bu, &u_Bnorm, gcge_ops);
...
PASE_Mg_set_pase_aux_matrix_by_pase_matrix(PASE_MG_SOLVER solver, PASE_INT i, PASE_INT j, void **u_j)
{
  ...
  void **A = solver->multigrid->A;
  void **B = solver->multigrid->B;
        ...
        PASE_MG_SOLVER solver, PASE_INT i, PASE_INT j)
{
  ...
  mv_e[0] = block_size;
  mv_s[1] = 0;
  mv_s[1] = block_size-nlock;
  // u_tmp(:,0:block_size-nlock) = A_h * u_h(:, nlock:block_size) 
  gcge_ops->MatDotMultiVec(A_h, u_j, u_tmp, mv_s, mv_e, gcge_ops);
  //i==1, j==0, 从j限制到1
  mv_s[0] = 0;
  mv_s[0] = block_size-nlock;
  mv_s[1] = nlock;
  mv_e[1] = block_size;
  PASE_INT error = PASE_MULTIGRID_RestrictFromItoJ(solver->multigrid, 
  ...
  gcge_ops->MultiVecInnerProd(u_j, u_tmp, A_aux_hh+nlock*num_aux_vec,
          "nonsym", mv_s, mv_e, num_aux_vec, 0, gcge_ops);
  //aux_hh对称化
  PASE_INT k = 0;//k表示列号
  PASE_INT l = 0;//l表示行号
...
PASE_Mg_set_pase_aux_vector(PASE_MG_SOLVER solver, PASE_INT cur_level)
{
  ...
  if(NULL == solver->aux_u[cur_level]) {
      solver->pase_ops->MultiVecCreateByMat(solver->aux_u[cur_level], block_size, 
              solver->multigrid->A_array[cur_level], solver->pase_ops);
#if 0
  ...
  mv_s[1] = nlock;
  mv_e[1] = block_size;
  solver->pase_ops->MultiVecAxpby(0.0, solver->aux_u[cur_level],
          0.0, solver->aux_u[cur_level], mv_s, mv_e, solver->pase_ops);
...
PASE_Mg_prolong_from_pase_aux_vector_to_pase_vector(PASE_MG_SOLVER solver, PASE_INT i, PASE_MultiVector aux_u_i, PASE_INT j, void **u_j)
{
    ...
    PASE_INT error = PASE_MULTIGRID_ProlongFromItoJ(solver->multigrid, 
          i, j, mv_s, mv_e, aux_u_i->u_H, u_j);
#if 0
  PASE_INT idx_block = 0;
...
PASE_Mg_smoothing_by_amg_hypre(void *mg_solver, char *PreOrPost)
{
  ...
  /*  TODO 下面的 for 循环求解线性方程组，改成 MultiVector 的形式 */
...
PASE_Mg_direct_solve_by_gcg(void *mg_solver)
{
  ...
  GCGE_SOLVER *gcge_pase_solver = GCGE_SOLVER_PASE_Create(aux_A, 
          aux_B, nev, eigenvalues, aux_u, solver->pase_ops);
  //求解
  GCGE_SOLVER_Solve(gcge_pase_solver);  
  //释放空间
  GCGE_SOLVER_Free_Some(&gcge_pase_solver);
#endif
